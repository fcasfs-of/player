function criarDivisor(config){const isLista=config&&Array.isArray(config.itens);const configs=isLista?config.itens:[config||{}];const espacamentoUniversal=(config&&config.espacamento)?config.espacamento:'20px';const margemLat=(config&&config.margemLateral)?config.margemLateral:'0px';const margemExt=(config&&config.margemExtremidades)?config.margemExtremidades:'0px';let htmlTotal='';configs.forEach(cfg=>{const{cor='#ccc',largura,altura,quantidade=1,orientacao='linha',tipo='fina',arredondado='0px',bordaTamanho='0px',bordaCor='transparent',espacamento='10px'}=cfg;let espessuraPadrao='2px';if(tipo==='media')espessuraPadrao='5px';if(tipo==='grossa'||tipo==='grosa')espessuraPadrao='10px';const isColuna=orientacao==='coluna';const dimEspessura=altura||espessuraPadrao;const dimComprimento=largura||(isColuna?'100px':'100%');let htmlContainer=`<div style="display:flex;flex-direction:${isColuna?'row':'column'};gap:${espacamento};justify-content:center;align-items:center;flex:${isLista?'1':'none'};width:${isLista?'auto':'100%'};box-sizing:border-box;">`;for(let i=0;i<quantidade;i++){const w=isColuna?dimEspessura:dimComprimento;const h=isColuna?dimComprimento:dimEspessura;htmlContainer+=`<div style="background-color:${cor};width:${w};height:${h};max-width:100%;border:${bordaTamanho} solid ${bordaCor};border-radius:${arredondado};box-sizing:border-box;"></div>`;}htmlContainer+='</div>';htmlTotal+=htmlContainer;});let htmlFinal='';if(isLista){htmlFinal=`<div style="display:flex;flex-direction:row;column-gap:${espacamentoUniversal};row-gap:0px;justify-content:center;align-items:center;width:100%;padding:${margemExt} ${margemLat};box-sizing:border-box;">${htmlTotal}</div>`;}else{htmlFinal=`<div style="padding:${margemExt} ${margemLat};width:100%;box-sizing:border-box;">${htmlTotal}</div>`;}if(config&&config.id){const alvo=document.getElementById(config.id);if(alvo){const temp=document.createElement('div');temp.innerHTML=htmlFinal;const novoNo=temp.firstElementChild;alvo.parentNode.insertBefore(novoNo,alvo);}}return htmlFinal;}
